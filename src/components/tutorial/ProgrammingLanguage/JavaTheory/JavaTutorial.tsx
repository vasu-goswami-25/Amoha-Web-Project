...
// Assuming Sidebar and MainContent are in the same directory as this file
import Sidebar from "./Sidebar"; 
import MainContent from "./MainContent";
import { tutorialContent, tutorialData, type SidebarItem } from "../../../../data/tutorial/JavaTheory/javaData";
// import { tutorialContent, tutorialData, type SidebarItem } from "../../../data/tutorial/CPPTheory/JavaTheory/javaData";
// import { tutorialData, tutorialContent, type SidebarItem } from "../../../data/tutorial/JavaTheory/javaData";

interface JavaTutorialProps {
  darkMode: boolean;
}

// Reusable logic to flatten data for navigation (copied from your CppTutorial.tsx)
...
const JavaTutorial: React.FC<JavaTutorialProps> = ({ darkMode }) => {

  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "java_home.asp");
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <Sidebar
        ...
        darkMode={darkMode}
        tutorialTitle="Java Theory" // Added prop for dynamic title
      />
